...
sums_of_the_numeric_parts = [sum_of_the_numeric_part(id) for id in ids]

max_sum = max(sums_of_the_numeric_parts) # wyłaniamy największą sumę
max_ids = [id for id in ids if sum_of_the_numeric_part(id) == max_sum] # wyłaniamy id, które mają największą sumę
